...
// TODO rewrite comment (copied from JGEngine)
...
class SGEngine {
	...
	var isApplet: Boolean = false // are we an applet?
	
	/** Init engine as application.  Passing SGPoint.None for width, height will
	 * result in a full-screen window without decoration.  Passing another
	 * value results in a regular window with decoration.
	 */
	def initEngine(viewportSize: SGPoint) {
//		if (viewportSize == SGPoint.None) {
//			val scrsize: Dimension = Toolkit.getDefaultToolkit().getScreenSize()
...
//		init();
	...
	def setCanvasSettings(boardSize: SGPoint, tileSize: SGPoint, foregroundCOlor: SGColor,
			backgroundColor: SGColor, messageFont: SGFont) {
...
//		el.tilex=tilex;
	...
	def setFrameRate(frameRate: Double, maxSkip: Double) {
//				el.setFrameRate(fps, maxframeskip);
	}
	
	def setColor(color: SGColor) {
//		if (buf_gfx!=null) setColor(buf_gfx,col);
	}

	def drawString(message: String, position: SGPoint, align: Alignment.Value) = {
//		drawString(buf_gfx, str, x,y, align, false);
	}
	
	def viewWidth: Int = 0 // el.viewnrtilesx*el.tilex; 
	def viewHeight: Int = 0 // el.viewnrtilesy*el.tiley;
}
